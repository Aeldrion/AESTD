...
with open("v1.mcfunction", mode="w") as file:
	...
	for i in range(branches_count):
		...
		with open("{v}_{n}.mcfunction".format(v=version, n=i), mode="w") as branch:
			...
			for j in range(len(branch_blocks)):
				branch.write("execute if block ~ ~ ~ {block} run scoreboard players set $block.id aestd {block_id}\n".format(block=branch_blocks[j], block_id=i*100+j))
